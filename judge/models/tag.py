...
class Tag(models.Model):
    code = models.CharField(
        max_length=30, verbose_name=_("Tag ID"), unique=True, db_index=True
    )
    name = models.CharField(max_length=100, verbose_name=_("Tag name"), db_index=True)
    group = models.ForeignKey(
        TagGroup,
        related_name="tags",
        verbose_name=_("Parent tag group"),
        on_delete=CASCADE,
    )
...
class TagProblem(models.Model):
    code = models.CharField(
        ...
        db_index=True,
        validators=[
            RegexValidator("^[a-zA-Z0-9_]+$", _("Problem code must be ^[a-zA-Z0-9_]+$"))
        ],
        help_text=_(
            "A short, unique code for the problem, used in the url after /tag/"
        ),
    )
    ...
    tag = models.ManyToManyField(
        Tag, through="TagData", related_name="tags", verbose_name=_("Tag")
    )
