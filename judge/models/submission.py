...
SUBMISSION_SEARCHABLE_STATUS = SUBMISSION_RESULT + tuple(
    [status for status in SUBMISSION_STATUS if status not in SUBMISSION_RESULT]
)
...
class Submission(models.Model):
    ...
    def can_see_detail(self, user):
        ...
        if self.problem.is_editable_by(user):
            ...
            not self.problem.is_public
            and user.has_perm("judge.suggest_new_problem")
            and self.problem.is_suggesting
        ):
            return True
            ...
            and self.problem.submission_set.filter(
                user_id=profile.id, result="AC", points=self.problem.points
            ).exists()
        ):
            return True
        elif (
        ...
        if (
            ...
            and user.profile.id in self.contest_object.editor_ids
        ):
            return True
