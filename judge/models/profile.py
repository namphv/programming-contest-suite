...
class Organization(models.Model):
    ...
    _pp_table = [
        pow(settings.VNOJ_ORG_PP_STEP, i) for i in range(settings.VNOJ_ORG_PP_ENTRIES)
    ]
...
class Profile(models.Model):
    ...
    organization_points = models.JSONField(
    ...
    def calculate_points(self):
        ...
        latest_submission_ids = (
            (
                Submission.objects.filter(
                    user=self, result="AC", case_points__gte=F("case_total")
                )
        ...
        latest_submissions = Submission.objects.filter(
            id__in=Subquery(latest_submission_ids)
        )

        result = latest_submissions.values(
            "contest_object__organizations__id"
        ...
        #           )
        result_dict = {
            item["contest_object__organizations__id"]: (
                item["total_points"],
