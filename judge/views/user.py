...
class UserAboutPage(UserPage):
    ...
    def get_context_data(self, **kwargs):
        ...
        timezone_offset = (
            pytz.timezone(user_timezone).utcoffset(datetime.utcnow()).seconds
        )

        submissions = (
            self.object.submission_set.annotate(
                date_only=Cast(
                    F("date") + timedelta(seconds=timezone_offset), DateField()
                )
            )
            .values("date_only")
...
class UserCommentPage(CustomUserMixin, DiggPaginatorMixin, ListView):
    ...
    def get_queryset(self):
        return Comment.get_newest_visible_comments(
            viewer=self.request.user, author=self.user, batch=2 * self.paginate_by
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["title"] = (
            self.title or _("Page %d of Comments") % context["page_obj"].number
        )
    ...
    def delete_comments(self, request, *args, **kwargs):
        ...
        for comment in Comment.get_newest_visible_comments(
            viewer=request.user, author=user, batch=2 * self.paginate_by
        ):
...
class UserList(
    ...
    def get_queryset(self):
        return (
            ...
            .prefetch_related(
                Prefetch("user", queryset=User.objects.only("username", "first_name"))
            )
            .prefetch_related(
                Prefetch(
                    "organizations",
                    queryset=Organization.objects.filter(is_unlisted=False).only(
                        "name", "id", "slug"
                    ),
                )
            )
...
def user_contributor_redirect(request):
    ...
    user = get_object_or_404(Profile, user__username=username)
    rank = Profile.objects.filter(
        is_unlisted=False, contribution_points__gt=user.contribution_points
    ).count()
    rank += Profile.objects.filter(
        is_unlisted=False,
        contribution_points__exact=user.contribution_points,
        id__lt=user.id,
    ).count()
    ...
    return HttpResponseRedirect(
        "%s%s#!%s"
        % (
            reverse("contributors_list"),
            "?page=%d" % (page + 1) if page else "",
            username,
        )
    )
