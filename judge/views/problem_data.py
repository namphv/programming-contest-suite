...
class ProblemDataForm(ModelForm):
    io_method = ChoiceField(
        choices=IO_METHODS,
        label=gettext_lazy("IO Method"),
        initial="standard",
        required=False,
        widget=Select2Widget(attrs={"style": "width: 200px"}),
    )
    io_input_file = CharField(
        max_length=100, label=gettext_lazy("Input from file"), required=False
    )
    io_output_file = CharField(
        max_length=100, label=gettext_lazy("Output to file"), required=False
    )
    checker_type = ChoiceField(
        choices=CUSTOM_CHECKERS, widget=Select2Widget(attrs={"style": "width: 200px"})
    )
    ...
    class Meta:
        ...
        help_texts = {
            "output_limit": _(
                "Can be left blank. In case the output can be too long (over 20MB), please set this."
            ),
        }
...
class ProblemCaseFormSet(
    formset_factory(
        ProblemCaseForm, formset=BaseModelFormSet, extra=0, max_num=1, can_delete=True
    )
):
    model = ProblemTestCase
    ...
    def get_context_data(self, **kwargs):
        ...
        if "username" in self.request.GET:
            ...
            subs = Submission.objects.filter(
                problem=self.object, user__user__username__in=usernames
            )
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            return generic_message(
                self.request,
                _("No such submissions"),
                _("Could not find any submissions."),
            )
...
class ProblemDataView(TitleMixin, ProblemManagerMixin):
    ...
    def check_valid(self, data_form, cases_formset):
        if not data_form.is_valid() or not cases_formset.is_valid():
            return False
        number_of_cases = cases_formset.total_form_count() - len(
            cases_formset.deleted_forms
        )
        if (
            number_of_cases > settings.VNOJ_TESTCASE_HARD_LIMIT
            and not self.request.user.has_perm("judge.create_mass_testcases")
        ):
            error = ValidationError(
                _("Too many testcases, number of testcases must not exceed %s")
                % settings.VNOJ_TESTCASE_HARD_LIMIT,
