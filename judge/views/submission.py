...
def submission_related(queryset):
    return (
        queryset.select_related(
            "user__user", "user__display_badge", "problem", "language"
        )
        .only(
            "id",
            ...
            "problem__testcase_result_visibility_mode",
            "user__username_display_override",
            "user__display_badge__name",
            "user__display_badge__mini",
        )
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            return generic_message(
                request,
                "Access denied",
                "This source cannot be viewed by normal users.",
                404,
            )
...
def SubmissionSourceDiff(request):
    ...
    if not first_sub.can_see_detail(request.user) or not second_sub.can_see_detail(
        request.user
    ):
    ...
    content_title = mark_safe(
        ...
        % {
            "first": format_html(
                '<a href="{0}">{1}</a>',
                reverse("submission_source", args=[first_id]),
                first_id,
            ),
            "second": format_html(
                '<a href="{0}">{1}</a>',
                reverse("submission_source", args=[second_id]),
                second_id,
            ),
        }
    )
...
def make_batch(batch, cases, statuses=None):
    ...
    if batch:
        ...
        if result["status"]:
            result["long_status"] = Submission.USER_DISPLAY_CODES.get(
                result["status"], ""
            )
    return result
...
class SubmissionStatus(SubmissionDetailBase):
    ...
    def get_queryset(self):
        return (
            super()
            .get_queryset()
            .select_related("contest", "contest_object", "contest__problem")
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        if (
            not submission.is_graded
            and len(statuses) > 0
            and statuses[-1].batch is not None
        ):
            context["batches"][-1]["not_graded"] = True

        context["statuses"] = combine_statuses(statuses, submission)
        context["can_view_test"] = submission.problem.is_testcase_accessible_by(
            self.request.user
        )
        ...
        context["can_view_testcase_status"] = (
            self.request.user.is_superuser
            or submission.problem.testcase_result_visibility_mode
            == ProblemTestcaseResultAccess.ALL_TEST_CASE
        )
        context["can_view_batch_status"] = (
            submission.problem.testcase_result_visibility_mode
            == ProblemTestcaseResultAccess.ONLY_BATCH_RESULT
        )

        context["can_view_feedback"] = (
            self.request.user.is_superuser or submission.problem.allow_view_feedback
        )
...
class SubmissionsListBase(DiggPaginatorMixin, TitleMixin, ListView):
    ...
    def _get_queryset(self):
        ...
        if self.selected_organization:
            organization_object = get_object_or_404(
                Organization, pk=self.selected_organization
            )
    ...
    def get_all_submissions_page(self):
        if self.in_contest and hasattr(self, "contest"):
            return reverse(
                "contest_all_submissions", kwargs={"contest": self.contest.key}
            )
    ...
    def get_searchable_status_codes(self):
        ...
        return [
            (key, value)
            for key, value in Submission.SEARCHABLE_STATUS
            if key not in hidden_codes
        ]
    ...
    def get_context_data(self, **kwargs):
        ...
        if hasattr(self, "contest"):
            context["best_submissions_link"] = reverse(
                ...
                kwargs={"problem": self.problem.code, "contest": self.contest.key},
            )
        else:
            context["best_submissions_link"] = reverse(
