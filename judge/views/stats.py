...
def submission_data(start_date, end_date, utc_offset):
    ...
    submissions = (
        ...
        .values("date_only", "result")
        .annotate(count=Count("result"))
        .values_list("date_only", "result", "count")
    )
    ...
    results = [
        (str(Submission.USER_DISPLAY_CODES[res]), count) for (res, count) in results
    ]
    ...
    queue_time = (
        ...
        .annotate(
            queue_time=Cast(F("judged_date") - F("date"), FloatField()) / 1000000.0
        )
        .order_by("queue_time")
        .values_list("queue_time", flat=True)
    )
    ...
    for date, result, count in submissions:
        result_data[result if result in result_order else "ERR"][
            days_labels.index(date.isoformat())
        ] += count
    ...
    queue_time_data = [
        (queue_time_labels[i], queue_time_count[i])
        for i in range(1, len(queue_time_labels))
    ]

    return {
        "by_day": get_stacked_bar_chart(
            days_labels, result_data, settings.DMOJ_STATS_SUBMISSION_RESULT_COLORS
        ),
...
def organization_data(start_date, end_date, utc_offset):
    submissions = (
        ...
        .filter(
            Q(problem__is_organization_private=True)
            | Q(contest_object__is_organization_private=True)
        )
    ...
    org_data = get_stacked_bar_chart(
        days_labels, result_data, settings.DMOJ_STATS_SUBMISSION_RESULT_COLORS
    )

    problems = (
        Problem.objects.filter(
            date__gte=start_date, date__lte=end_date, is_organization_private=True
        )
        .annotate(date_only=Cast(F("date") + utc_offset, DateField()))
        .values("date_only")
        .annotate(count=Count("date_only"))
        .values_list("date_only", "count")
    )
