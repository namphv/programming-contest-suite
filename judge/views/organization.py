...
class OrganizationMixin(object):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["meta_description"] = self.object.about[
...
class OrganizationUsers(
    ...
    def get_queryset(self):
        ...
        users = self.object.members.filter(is_unlisted=False).annotate(
            org_points=Cast(
                RawSQL("JSON_EXTRACT(organization_points, %s)", (json_path_points,)),
                FloatField(),
            ),
            submission_count=RawSQL(
                "IFNULL(CAST(JSON_UNQUOTE(JSON_EXTRACT(organization_points, %s)) AS UNSIGNED), 0)",
                (json_path_submissions,),
            ),
        )
    ...
    def handle(self, request, org, profile):
        ...
        if org.is_admin(profile):
            return generic_message(
                request,
                _("Leaving organization"),
                _("You cannot leave an organization you own."),
            )
...
class CreateOrganization(PermissionRequiredMixin, TitleMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        if self.has_permission():
            if (
                self.request.user.profile.admin_of.count()
                >= settings.VNOJ_ORGANIZATION_ADMIN_LIMIT
                and not self.request.user.has_perm("spam_organization")
            ):
                return render(
                    request,
                    "organization/create-limit-error.html",
                    {
            ...
            return generic_message(
                request,
                _("Can't create organization"),
                _("You are not allowed to create new organizations."),
                status=403,
            )
...
class CustomOrganizationMixin(object):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["meta_description"] = self.organization.about[
            : settings.DESCRIPTION_MAX_LENGTH
        ]
        return context
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if (
            ...
            and not any(
                self.request.user.has_perm(perm) for perm in self.permission_bypass
            )
        ):
            return generic_message(
                request,
...
class CustomAdminOrganizationMixin(CustomOrganizationMixin):
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.can_edit_organization():
            return super(CustomAdminOrganizationMixin, self).dispatch(
                request, *args, **kwargs
            )
        raise PermissionDenied
...
class OrganizationHome(TitleMixin, CustomOrganizationMixin, PostListBase):
    ...
    def get_queryset(self):
            ...
            if not self.can_edit_organization():
                if self.request.profile in self.object:
                    ...
                    queryset = queryset.filter(
                        publish_on__lte=timezone.now(), visible=True
                    )
                else:
                ...
                queryset = queryset.filter(
                    Q(visible=True) | Q(authors=self.request.profile)
                )
        ...
        return queryset.order_by("-sticky", "-publish_on").prefetch_related(
            "authors__user"
        )

    def get_context_data(self, **kwargs):
        context = super(OrganizationHome, self).get_context_data(**kwargs)
        context["first_page_href"] = reverse(
            "organization_home", args=[self.object.pk, self.object.slug]
        )
        context["title"] = self.object.name
        ...
        if (
            context["is_member"]
            or user.has_perm("judge.see_organization_problem")
        ...
        see_private_contest = user.has_perm(
        ...
        if context["is_member"] or see_private_contest:
            ...
            context["new_contests"] = new_contests[
...
class ContestListOrganization(CustomOrganizationMixin, ContestList):
    ...
    def _get_queryset(self):
        ...
        query_set = query_set.filter(
            is_organization_private=True, organizations=self.organization
        )
        return query_set
...
class ProblemCreateOrganization(CustomAdminOrganizationMixin, ProblemCreate):
    ...
    def get_initial(self):
        ...
        initial["code"] = (
            "".join(x for x in self.organization.slug.lower() if x.isalpha()) + "_"
        )
        return initial
...
class BlogPostCreateOrganization(
    CustomAdminOrganizationMixin, PermissionRequiredMixin, BlogPostCreate
):
    ...
    def form_valid(self, form):
        with revisions.create_revision(atomic=True):
            ...
            post.slug = "".join(
                x for x in self.organization.slug.lower() if x.isalpha()
            )  # Initial post slug
...
class CSVImportForm(forms.Form):
    csv_file = forms.FileField(
        label=_("CSV File"),
        help_text=_(
            "Upload CSV file with columns: email, full_name, password (max 2MB, 100 users)"
        ),
...
class OrganizationUserImportView(CustomAdminOrganizationMixin, FormView):
    ...
    def get_form_kwargs(self):
        kwargs = super(
            FormView, self
        ).get_form_kwargs()  # Skip CustomAdminOrganizationMixin's get_form_kwargs
        return kwargs
    ...
    def process_csv_file(self, csv_file):
        ...
        if not all(header in csv_reader.fieldnames for header in required_headers):
            raise ValueError(
                _(
                    "CSV must contain columns: email, full_name, password. Username column is optional."
                )
            )
        ...
        if len(rows) > 100:
            raise ValueError(
                _("Maximum 100 users allowed per import. Your file has %d rows.")
                % len(rows)
            )
                        ...
                        errors.append(
                            {
                                "row": row_num,
                                "error": _(
                                    "Missing required fields (email, full_name, password)"
                                ),
                            }
                        )
                        continue
                            ...
                            skipped_users.append(
                                {
                                    "row": row_num,
                                    "email": email,
                                    "reason": _("Username already exists: %s")
                                    % username,
                                }
                            )
                            continue
...
class OrganizationUserImportTemplateView(CustomAdminOrganizationMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        writer.writerow(
            ["admin@school.edu.vn", "Quản Trị Viên", "password123", "admin"]
        )
        writer.writerow(
            ["teacher1@school.edu.vn", "Nguyễn Văn Giáo", "giaovien2024", ""]
        )  # Empty username - will be generated
        writer.writerow(
            [
                "student.2024@school.edu.vn",
                "Trần Thị Học Sinh",
                "hocsinh123",
                "student2024",
            ]
        ...
        writer.writerow(
            ["pham.van.nam@school.edu.vn", "Phạm Văn Nam", "matkhau456", ""]
        )  # Empty username - will be generated
