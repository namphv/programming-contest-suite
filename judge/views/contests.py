...
from judge.utils.problems import (
    ...
    user_attempted_ids,
    user_completed_ids,
    contest_user_completed_ids,
    contest_user_attempted_ids,
)
    ...
    def get_queryset(self):
        if self.hide_private_contests is not None:
            if "hide_private_contests" in self.request.GET:
                self.hide_private_contests = self.request.session[
                    "hide_private_contests"
                ] = self.request.GET.get("hide_private_contests").lower() == "true"
            else:
                self.hide_private_contests = self.request.session.get(
                    "hide_private_contests", False
                )
...
class ContestList(
    ...
    def get_queryset(self):
        ...
        if "search" in self.request.GET:
            self.search_query = search_query = " ".join(
                self.request.GET.getlist("search")
            ).strip()
            if search_query:
                query_set = query_set.filter(
                    Q(key__icontains=search_query) | Q(name__icontains=search_query)
                )
        return query_set
...
class ContestDetail(ContestMixin, TitleMixin, CommentedDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        points_list = list(
            self.object.contest_problems.values_list("points").order_by("order")
        )
        ...
        context["completed_problem_ids"] = (
            contest_user_completed_ids(self.request.profile, self.object)
            if authenticated
            else []
        )
        context["attempted_problem_ids"] = (
            contest_user_attempted_ids(self.request.profile, self.object)
            if authenticated
            else []
        )
...
class ContestClone(
    ...
    def get_contest_problem_formset(self):
        if self.request.POST:
            form_set = ProposeContestProblemFormSet(
...
class ContestRegister(LoginRequiredMixin, ContestMixin, SingleObjectMixin, View):
    ...
    def register_contest(self, request, access_code=None):
        ...
        if contest.ended:
            return generic_message(
                request, _("Contest has ended"), _('"%s" has ended.') % contest.name
            )
        else:
            if self.is_editor or self.is_tester:
                return generic_message(
        ...
        if contest.ended:
            ...
            try:
                participation = ContestParticipation.objects.get(
                    contest=contest,
                    user=profile,
                    virtual=(SPECTATE if can_only_spectate else LIVE),
                )
            except ContestParticipation.DoesNotExist:
                if (
                    contest.require_registration
                    and not contest.can_register
                    and not can_only_spectate
                ):
                    return generic_message(
                        request,
                        _("Not registered"),
...
class ContestStats(TitleMixin, ContestMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        queryset = Submission.objects.filter(
            contest_object=self.object, date__gt=self.object.start_time
        )
...
def make_contest_ranking_profile(
    contest, participation, contest_problems, first_solves, frozen=False
):
    def display_user_problem(contest_problem):
        ...
        try:
            return contest.format.display_user_problem(
                participation, contest_problem, first_solves, frozen
            )
    ...
    return ContestRankingProfile(
        ...
        if not frozen
        else participation.frozen_tiebreaker,
...
def base_contest_ranking_queryset(contest):
    return (
        contest.users.filter(virtual__gt=ContestParticipation.SPECTATE)
        .prefetch_related(
            Prefetch(
                "user__organizations",
...
def contest_ranking_list(contest, problems, frozen=False):
    return base_contest_ranking_list(
        contest, problems, base_contest_ranking_queryset(contest), frozen=frozen
    )
...
class ContestRankingBase(ContestMixin, TitleMixin, DetailView):
    ...
    def get(self, request, *args, **kwargs):
        if "raw" in request.GET:
            ...
            return HttpResponse(
                self.get_rendered_ranking_table(), content_type="text/plain"
            )
...
class ContestRanking(ContestRankingBase):
    ...
    def cache_key(self):
        return (
            f"contest_ranking_cache_{self.object.key}_{self.show_virtual}_{self.is_frozen}_"
    ...
    def bypass_cache_ranking(self):
        return (
            self.object.scoreboard_cache_timeout == 0
            or self.can_edit
            or (
                self.request.user.is_authenticated
                and not self.object.can_see_full_scoreboard(self.request.user)
            )
        )
    ...
    def get_full_ranking_list(self):
        ...
        return get_contest_ranking_list(
            ...
            ranking_list=partial(
                base_contest_ranking_list, queryset=queryset, frozen=self.is_frozen
            ),
        )
    ...
    def get_rendered_ranking_table(self):
        ...
        if rendered_ranking_table is None:
            ...
            cache.set(
                ...
                self.object.scoreboard_cache_timeout,
            )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["cache_timeout"] = (
            0 if self.bypass_cache_ranking else self.object.scoreboard_cache_timeout
        )
        return context
...
class ContestPublicRanking(ContestRanking):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not ranking_access_code or ranking_access_code != request.GET.get("code"):
            return generic_message(
                request,
                _("Ranking access code required"),
                _(
                    "You need to provide a valid ranking access code to access this page."
                ),
            )
...
class ContestOfficialRanking(ContestRankingBase):
    ...
    def get_ranking_list(self):
        ...
        for user in users:
            ...
            user["problem_cells"] = [
                display_points(points) for points in user["scores"]
            ]
...
class ContestPrepareData(ContestDataMixin, TitleMixin, SingleObjectMixin, FormView):
    ...
    def can_prepare_data(self):
        return (
            ...
            or self.object.data_last_downloaded
            + settings.DMOJ_CONTEST_DATA_DOWNLOAD_RATELIMIT
            < self._now
    ...
    def form_valid(self, form):
        ...
        self.object.save()
        status = prepare_contest_data.delay(
            self.object.id, json.dumps(form.cleaned_data)
        )
...
class ContestDownloadData(ContestDataMixin, SingleObjectMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        if hasattr(settings, "DMOJ_CONTEST_DATA_INTERNAL"):
            url_path = "%s/%s.zip" % (
                settings.DMOJ_CONTEST_DATA_INTERNAL,
                self.object.id,
            )
        else:
            url_path = None
        ...
        response["Content-Disposition"] = (
            "attachment; filename=%s-data.zip" % self.object.key
        )
        return response
