    ...
    def dispatch(self, request, *args, **kwargs):
        if not request.user.is_authenticated or (not request.profile.can_tag_problems):
            return generic_message(
                request, _("Cannot tag"), _("You are not allowed to tag problem.")
            )
...
class TagFindProblem(View):
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            return HttpResponseRedirect(
                "%s?problem_url=%s" % (reverse("tagproblem_create"), problem_url)
            )
...
class TagProblemCreate(LoginRequiredMixin, TagAllowingMixin, TitleMixin, FormView):
    ...
    def form_valid(self, form):
        try:
            ...
            try:
                API = OJAPI()
                api_method = API.__getattribute__(
                    problem_data["judge"] + "ProblemAPI"
                )  # Get method based on judge
            ...
            with revisions.create_revision(atomic=True):
                problem = TagProblem(
                    code=problem_data["codename"],
                    name=api_problem_data["title"],
                    link=url,
...
class TagProblemAssign(
    ...
    def get_content_title(self):
        return mark_safe(
            escape(_("Assign new tags for %s"))
            % format_html(
                '<a href="{0}">{1}</a>',
    ...
    def form_valid(self, form):
        ...
        for tag in tags:
            ...
            try:
                with revisions.create_revision(atomic=True):
                    tag_data = TagData(
                        assigner=self.request.profile, problem=self.object, tag=tag
                    )
                    tag_data.save()
...
class TagProblemDetail(TagProblemMixin, TitleMixin, CommentedDetailView):
    ...
    def get_content_title(self):
        return mark_safe(
            format_html('<a href="{0}">{1}</a>', self.object.link, self.object.name)
        )
    ...
    def get_queryset(self):
        queryset = TagData.objects.select_related("tag", "assigner__user").only(
            "id",
            "problem",
            "tag",
            "assigner__user__username",
            "assigner__display_rank",
            "assigner__rating",
        )

        return (
            super(TagProblemDetail, self)
            .get_queryset()
