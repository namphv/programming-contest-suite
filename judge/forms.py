    ...
    def clean_about(self):
        if "about" in self.changed_data and not self.instance.has_enough_solves:
            raise ValidationError(
                _(
                    "You must solve at least %d problems before you can update your profile."
                )
...
class LanguageLimitForm(ModelForm):
    ...
    def clean_time_limit(self):
        ...
        if (
            timelimit
            and timelimit > settings.VNOJ_PROBLEM_TIMELIMIT_LIMIT
            and not has_high_perm
        ):
...
class ProblemEditForm(ModelForm):
    statement_file = forms.FileField(
        ...
        help_text=_("Maximum file size is %s.")
    ...
    def __init__(self, *args, **kwargs):
        ...
        if org_pk is None:
            ...
            self.fields["testers"].help_text = _(
                "If private, only these users may see the problem."
            )
            self.fields["testers"].widget.data_view = None
            self.fields["testers"].widget.data_url = reverse(
                "organization_profile_select2", args=(org_pk,)
            )
    ...
    class Meta:
        ...
        help_texts = {
            ...
            "points": _(
                "Points awarded for problem completion. From 0 to 2. "
        ...
        error_messages = {
            "code": {
                "invalid": _(
                    "Only accept alphanumeric characters (a-z, 0-9) and underscore (_)"
                ),
            },
        }


class ProposeProblemSolutionFormSet(
    inlineformset_factory(Problem, Solution, form=ProposeProblemSolutionForm)
):
    pass


class LanguageLimitFormSet(
    inlineformset_factory(
        Problem, LanguageLimit, form=LanguageLimitForm, can_delete=True
    )
):
    pass
...
class ContestDownloadDataForm(Form):
    submission_download = BooleanField(
        required=False, initial=True, label=_("Download submissions?")
    )
    submission_problem_glob = CharField(
        initial="*", label=_("Filter by problem code glob:"), max_length=100
    )
...
class ProblemSubmitForm(ModelForm):
    ...
    def check_submission(self):
        ...
        if (
            (source != "" and content is not None)
            or (source == "" and content is None)
            or (source != "" and lang_obj.file_only)
            or (content == "" and not lang_obj.file_only)
        ):
            raise forms.ValidationError(
                _("Source code/file is missing or redundant. Please try again")
            )
...
class TagProblemCreateForm(Form):
    problem_url = forms.URLField(
        max_length=200,
        label=_("Problem URL"),
        help_text=_("Full URL to the problem, e.g. https://oj.vnoi.info/problem/post"),
...
class OrganizationForm(ModelForm):
    class Meta:
        ...
        if HeavyPreviewPageDownWidget is not None:
            widgets = {
                "about": HeavyPreviewPageDownWidget(
...
class BlogPostForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "summary": MartorWidget(
                attrs={"data-markdownfy-url": reverse_lazy("blog_preview")}
            ),
            "publish_on": DateTimeInput(
...
class TutorialForm(ModelForm):
    ...
    def clean_youtube_url(self):
        ...
        if not video_id:
            raise ValidationError(
                _("Invalid YouTube URL. Please provide a valid YouTube video URL.")
            )
    ...
    class Meta:
        ...
        widgets = {
            "content": MartorWidget(
                attrs={"data-markdownfy-url": reverse_lazy("tutorial_preview")}
            ),
            "summary": MartorWidget(
                attrs={"data-markdownfy-url": reverse_lazy("tutorial_preview")}
            ),
            ...
            "slide_deck_url": forms.URLInput(
                attrs={
                    "placeholder": "https://docs.google.com/presentation/d/[ID]/edit or embed URL"
                }
            ),
            ...
            "python_code": AceWidget(
                mode="python", theme="github", width="100%", height="400px"
            ),
        ...
        help_texts = {
            "content": _(
                "Main tutorial content in Markdown format. You can embed code blocks and interactive elements."
            ),
            "summary": _(
                "Brief summary of the tutorial (optional, will use content preview if empty)."
            ),
            "slide_deck_url": _(
                "URL to slide deck. Supports Google Slides, PowerPoint Online, SlideShare. Regular sharing URLs will be auto-converted to embed format."
            ),
            "youtube_url": _(
                "YouTube video URL. Supports various YouTube URL formats (watch, embed, share). Will be auto-converted to embed format."
            ),
            "python_code": _(
                "Python code examples that will be displayed with syntax highlighting"
            ),
            "scratch_image": _(
                "Image to display in the Scratch tab (JPG, PNG, GIF supported)"
            ),
        }
...
class ContestForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Configure tutorial field widget for organization-specific data if needed
        if "tutorial" in self.fields:
            if org_pk:
                # For organization contests, pass org_pk as a parameter
                self.fields["tutorial"].widget.attrs["data-org-pk"] = org_pk
    ...
    def clean(self):
        ...
        if (
            end_time
            and start_time
            and (end_time - start_time).days > settings.VNOJ_CONTEST_DURATION_LIMIT
    ...
    class Meta:
        ...
        widgets = {
            ...
            "tutorial": HeavySelect2Widget(
                data_view="tutorial_select2",
                attrs={"style": "width: 100%"},
            ),
