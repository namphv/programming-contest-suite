...
class JudgeList(object):
        ...
        with self.lock:
            ...
            while node:
                if isinstance(node.value, PriorityMarker):
                    ...
                    if judge.name not in banned_judges and judge.can_judge(
    ...
    def judge(
        self, id, problem, language, source, judge_id, priority, banned_judges=[]
    ):
        with self.lock:
            ...
            candidates = [
                judge
                for judge in self.judges
                if judge.name not in banned_judges
            ...
            if available:
                ...
                try:
                    ...
                    self.judges.discard(judge)
                    return self.judge(
                        id, problem, language, source, judge_id, priority, banned_judges
                    )
            else:
