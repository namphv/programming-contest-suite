...
def get_visible_content(archive, filename):
    if archive.getinfo(filename).file_size <= settings.VNOJ_TESTCASE_VISIBLE_LENGTH:
        ...
        data = (
            archive.open(filename).read(settings.VNOJ_TESTCASE_VISIBLE_LENGTH) + b"..."
        )
        ...
        def make_checker(case):
            if case.checker == "bridged":
                ...
                if checker_ext not in ["cpp", "pas", "java"]:
                    raise ProblemDataError(
                        _("Only C++, Pascal, or Java checkers are supported.")
                    )
            ...
            if case.grader == "standard":
                if grader_args.get("io_method") == "file":
                    if (
                        ...
                        raise ProblemDataError(
                            _("You must specify both input and output files.")
                        )

                    if not isinstance(
                        grader_args["io_input_file"], str
                    ) or not isinstance(grader_args["io_output_file"], str):
