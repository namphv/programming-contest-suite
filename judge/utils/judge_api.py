...
class OJAPI:
        ...
        if problemset is None:
            ...
            for problem in problemset_data:
                ...
                if code in problemset:
                    raise ValueError(
                        "Problem code %s appeared twice in the problemset." % code
                    )
                problemset[code] = {
    ...
    def KattisProblemAPI(codename):
        ...
        verification = requests.get(
            url="https://open.kattis.com/problems/%s" % codename, timeout=_TIMEOUT
        ).status_code
        if verification != 200:
            return None
    ...
    def CodeforcesGymProblemAPI(codename):
        ...
        if contestset is None or contestid > max(contestset):
            ...
            for contest in contestset_data["result"]:
                contestset.append(str(contest["id"]))
            cache.set(
                "OJAPI_data_CodeforcesGym", contestset, settings.OJAPI_CACHE_TIMEOUT
            )
