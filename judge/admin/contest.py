...
class ContestProblemInline(SortableInlineAdminMixin, admin.TabularInline):
    ...
    def rejudge_column(self, obj):
        if obj.id is None:
            return ""
        return format_html(
            '<a class="button rejudge-link" href="{0}">{1}</a>',
    ...
    def rescore_column(self, obj):
        if obj.id is None:
            return ""
        return format_html(
            '<a class="button rescore-link" href="{}">Rescore</a>',
...
class ContestAnnouncementInline(admin.StackedInline):
    ...
    def resend(self, obj):
        ...
        return format_html(
            '<a class="button resend-link" href="{}">Resend</a>',
...
class ContestAdmin(NoBatchDeleteMixin, VersionAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if form.changed_data and any(
            ...
            for f in ("frozen_last_minutes", "format_config", "format_name")
        ):
    ...
    def get_urls(self):
        return [
            ...
            path(
                "<int:contest_id>/rejudge/<int:problem_id>/",
                self.rejudge_view,
                name="judge_contest_rejudge",
            ),
            path(
                "<int:contest_id>/rescore/<int:problem_id>/",
                self.rescore_view,
                name="judge_contest_rescore",
            ),
            path(
                "<int:contest_id>/resend/<int:announcement_id>/",
                self.resend_view,
                name="judge_contest_resend",
            ),
