...
from django.contrib.flatpages.admin import (
    FlatPageAdmin as OldFlatPageAdmin,
    FlatpageForm as OldFlatpageForm,
)
...
class BlogPostForm(ModelForm):
    ...
    class Meta:
        widgets = {
            ...
            "organization": AdminHeavySelect2Widget(
                data_view="organization_select2", attr={"style": "width: 100%"}
            ),
        }
...
class TutorialAdmin(VersionAdmin):
    fieldsets = (
        (
            None,
            {
    ...
    def has_change_permission(self, request, obj=None):
        if obj is None:
            return request.user.has_perm("judge.change_tutorial")
        return obj.is_editable_by(request.user)
