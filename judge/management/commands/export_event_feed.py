...
def fill_info(contest: Contest, root: ET.Element):
    ...
    ET.SubElement(info, "length").text = str(
        contest.time_limit or contest.contest_window_length
    )
    ET.SubElement(info, "penalty").text = str(contest.format.config.get("penalty", 0))
    ET.SubElement(info, "started").text = (
        "True" if timezone.now() >= contest.start_time else "False"
    )
    if contest.frozen_last_minutes:
        ET.SubElement(info, "scoreboard-freeze-length").text = str(
            timedelta(minutes=contest.frozen_last_minutes)
        )
...
def fill_run(
    ...
    for contest_sub in (
        ContestSubmission.objects.filter(
            participation__contest=contest, participation__virtual=0
        )
        ...
        ET.SubElement(run, "time").text = str(
            (sub.date - contest.start_time).total_seconds()
        )
