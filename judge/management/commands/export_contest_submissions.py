...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        users = contest.users.filter(virtual=ContestParticipation.LIVE).select_related(
            "user__user"
        )
        ...
        for user in users:
            ...
            problems = set()
            submissions = (
                user.submissions.order_by("-id")
                .select_related(
                    "problem__problem", "submission__source", "submission__language"
                )
                .values_list(
            ...
            for problem, source, ext, sub_id in submissions:
                ...
                if problem not in problems:  # Last submission
                    ...
                    with open(
                        os.path.join(user_history_dir, f"{problem}_{sub_id}.{ext}"), "w"
                    ) as f:
                        f.write(source)
